...
optionsFinite = hampathset(options, 'derivative', 'finite');
ysol    = [1; 1; 2];
jFinite = sjac(ysol, optionsFinite, par);

dxdyFinite  = jFinite(1:2,1:2)
y1          = ysol(1);
y2          = ysol(2);
dt1dy1      = -y2/y1;
dt1dy2      = 1/y1;
...
e = ysol - [p0_solution; tf_solution];

fprintf('\n');
fprintf(['      Initial guess                 : y_guess  = [%g, %g, %g]\n'], y_guess);
fprintf(['      Solution trouvée              : p0       = [%g, %g]\n'], ysol(1:2));
fprintf(['      Solution trouvée              : tf       = %g\n'], ysol(3));
fprintf(['      Ecart à la solution           : e        = [%g, %g, %g]\n'], e);
fprintf(['      Valeur de la fonction de tir  : S(p0)    = [%g, %g, %g]\n'], Ssol);
...
e = ysolMatlab - [p0_solution; tf_solution];
...
subplot(nbFigs{:},4); hold on; xlim([tout(1) tout(end)]); daxes(0, 0, 'k--'); plot(tout, z(n+2,:), 'b', 'LineWidth', LW);
