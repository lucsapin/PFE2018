...
Subroutine sfun(ny,y,npar,par,s)
    ...
    call hfun(tspan(2),n,zf,iarc,npar,par,hf)

    ! equations de tir
    s(1)     = zf(1) - par(3)   ! x1(tf) - x1_f
    s(2)     = zf(2) - par(4)   ! x2(tf) - x2_f
    s(3)     = hf + pzero       ! h(tf) = -p^0
    s(4)     = z0(n+2)          ! H = H0 + u H1 => H1(t1) = p2(t1) = 0 : commutation
