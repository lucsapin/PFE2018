...
// CONTINUITY FOR PIECEWISE LINEAR CONTROL
// this will add (intervals-1) conditions, adjust dim_boundary_conditions manually !
// also set the constraints equal to 0 in the bounds !
if (dim_boundary_conditions > 7)
{
  int paramcontrol_type = constants[8];
  int paramcontrol_degree = constants[9];
  int paramcontrol_intervals = constants[10];
  junctionconditions(paramcontrol_type, paramcontrol_intervals, paramcontrol_degree, &optimvars[1], fixed_initial_time, fixed_final_time, &boundary_conditions[7]);
}
}
