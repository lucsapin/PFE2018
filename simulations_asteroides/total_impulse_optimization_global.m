...
function total_impulse_optimization_global(numAsteroid, numOptiOutbound, destination)
...
optionsFmincon      = optimoptions('fmincon','display','final-detailed','Algorithm','interior-point', ...
                                    'ConstraintTolerance', 1e-6, 'OptimalityTolerance', 1e-6, 'StepTolerance', 1e-12, 'FunctionTolerance', 1e-12, ...
...
% Criterion
F0                  = @(X) total_impulse_criterion(X, xOrb_epoch_t0_Ast, ratio, scaling, time_min_on_ast, time_max_on_ast);

% Solver
rng default % For reproducibility
gs      = GlobalSearch;
problem = createOptimProblem('fmincon','x0', X0, 'lb', LB, 'ub', UB, 'nonlcon', nonlc, 'objective', F0, 'options', optionsFmincon);
[Xsol,Fsol,exitflag,output,~] = run(gs,problem);
