...
#include "header_dynamics"
{
	...
	Tdouble y4    = state[22];
	Tdouble z4    = state[23];
	Tdouble xdot4 = state[24];
	Tdouble ydot4 = state[25];
	Tdouble zdot4 = state[26];
	Tdouble m4    = state[27];
	Tdouble x5    = state[28];
	Tdouble y5    = state[29];
	Tdouble z5    = state[30];
	Tdouble xdot5 = state[31];
	Tdouble ydot5 = state[32];
	Tdouble zdot5 = state[33];
	Tdouble m5    = state[34];
...
// TROISIEME ARC
	state_dynamics[14] = dt3 * xdot3;
	state_dynamics[15] = dt3 * ydot3;
	state_dynamics[16] = dt3 * zdot3;
...
// QUATRIEME ARC
	...
	state_dynamics[23]  = dt4 * zdot4;
	state_dynamics[24] = dt4 * (2*ydot4 + x4 - (1-mu)*(x4+mu)/rho41/rho41/rho41 - mu*(x4-1+mu)/rho42/rho42/rho42 - muS*(x4-rS*cos(theta4))/rhoS4/rhoS4/rhoS4 - muS*cos(theta4)/rS/rS);
	state_dynamics[25] = dt4 * (-2*xdot4 + y4 - (1-mu)*y4/rho41/rho41/rho41 - mu*y4/rho42/rho42/rho42 - muS*(y4-rS*sin(theta4))/rhoS4/rhoS4/rhoS4 - muS*sin(theta4)/rS/rS);
	state_dynamics[26] = dt4 * (-(1-mu)*z4/rho41/rho41/rho41 - mu*z4/rho42/rho42/rho42 - muS*z4/rhoS4/rhoS4/rhoS4);
	state_dynamics[27] = dt4 * 0;
// CINQUIEME ARC
	state_dynamics[28] = dtf * xdot5;
	state_dynamics[29] = dtf * ydot5;
	state_dynamics[30] = dtf * zdot5;
